...
public class DoublyLinkedList {
    ...
    public int getMiddleNode(){
        Node leftPoint = header;
        Node rightPoint = tailer;
        int data = -1;

        while(leftPoint!=rightPoint || (leftPoint.getNext()!=rightPoint)){
            if(leftPoint==rightPoint){
                data = leftPoint.getElement();
                break;
            }
            if(leftPoint.getNext()==rightPoint){
                data = leftPoint.getElement();
                break;
            }
            leftPoint = leftPoint.getNext();
            rightPoint = rightPoint.getPrev();
        }
